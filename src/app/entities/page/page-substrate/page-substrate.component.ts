...
import { PageSubstarateStyle } from './types';
import { PageSubstarateStyles } from './enums';
 ...
 */
@Component({
  selector: 'x-page-substrate',
  imports: [CommonModule],
  templateUrl: './page-substrate.component.html',
  styleUrl: './page-substrate.component.scss',
  encapsulation: ViewEncapsulation.Emulated,
})
export class PageSubstrateComponent {
  private static __id: number = 0;
  private static get nextId() {
    const id = PageSubstrateComponent.__id = PageSubstrateComponent.__id + 1 === Number.MAX_SAFE_INTEGER ? 0 : PageSubstrateComponent.__id + 1;
    return id;
  }
  ...
  type = input<PageSubstarateStyle>(PageSubstarateStyles.NONE);
  ...
  constructor() {
    this._id = PageSubstrateComponent.nextId;
    ...
    effect(() => {
      ...
      if (shape) {
        switch (type) {
          case PageSubstarateStyles.STROKE: {
            shape.setAttribute(STROKE, `url(#${STROKE_GRADIENT_NAME}${this._id})`);
            break;
          }
          case PageSubstarateStyles.NONE:
          default:
